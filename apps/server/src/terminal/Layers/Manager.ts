...
import { makeKeyedCoalescingWorker } from "@codewithme/shared/KeyedCoalescingWorker";
import {
  Data,
  Effect,
  Encoding,
...
function legacySafeProjectId(projectId: string): string {
...
function toSessionKey(projectId: string, terminalId: string): string {
  return `${projectId}\u0000${terminalId}`;
}

function shouldExcludeTerminalEnvKey(key: string): boolean {
  const normalizedKey = key.toUpperCase();
  if (normalizedKey.startsWith("CODEWITHME_")) {
    return true;
  }
...
export const makeTerminalManagerWithOptions = Effect.fn("makeTerminalManagerWithOptions")(
  function* (options: TerminalManagerOptions) {
    ...
    const projectLocksRef = yield* SynchronizedRef.make(new Map<string, Semaphore.Semaphore>());
    ...
    const historyPath = (projectId: string, terminalId: string) => {
      const projectPart = toSafeProjectId(projectId);
      if (terminalId === DEFAULT_TERMINAL_ID) {
        return path.join(logsDir, `${projectPart}.log`);
      }
      return path.join(logsDir, `${projectPart}_${toSafeTerminalId(terminalId)}.log`);
    };

    const legacyHistoryPath = (projectId: string) =>
      path.join(logsDir, `${legacySafeProjectId(projectId)}.log`);

    const toTerminalHistoryError =
      (operation: "read" | "truncate" | "migrate", projectId: string, terminalId: string) =>
      (cause: unknown) =>
    ...
    const getProjectSemaphore = (projectId: string) =>
      SynchronizedRef.modifyEffect(projectLocksRef, (current) => {
    ...
    const withProjectLock = <A, E, R>(
      ...
      Effect.flatMap(getProjectSemaphore(projectId), (semaphore) => semaphore.withPermit(effect));
    ...
    const startKillEscalation = Effect.fn("terminal.startKillEscalation")(function* (
      ...
      const fiber = yield* runKillEscalation(process, projectId, terminalId).pipe(
        Effect.ensuring(
    ...
    >({
      ...
      process: Effect.fn("terminal.persistHistoryWorker")(function* (sessionKey, request) {
        ...
        const [projectId, terminalId] = sessionKey.split("\u0000");
        if (!projectId || !terminalId) {
          return;
        }

        yield* fileSystem.writeFileString(historyPath(projectId, terminalId), request.history).pipe(
          Effect.catch((error) =>
    ...
    const queuePersist = Effect.fn("terminal.queuePersist")(function* (
      ...
      yield* persistWorker.enqueue(toSessionKey(projectId, terminalId), {
        history,
        immediate: false,
      });
    });
    ...
    const flushPersist = Effect.fn("terminal.flushPersist")(function* (
      ...
      yield* persistWorker.drainKey(toSessionKey(projectId, terminalId));
    });
    ...
    const persistHistory = Effect.fn("terminal.persistHistory")(function* (
      ...
      yield* flushPersist(projectId, terminalId);
    });
    ...
    const readHistory = Effect.fn("terminal.readHistory")(function* (
      ...
      const nextPath = historyPath(projectId, terminalId);
      if (
        yield* fileSystem
          .exists(nextPath)
          .pipe(Effect.mapError(toTerminalHistoryError("read", projectId, terminalId)))
      ) {
        ...
        if (capped !== raw) {
          yield* fileSystem
            ...
            .pipe(Effect.mapError(toTerminalHistoryError("truncate", projectId, terminalId)));
        }
        return capped;
      }
      ...
      const legacyPath = legacyHistoryPath(projectId);
      if (
        !(yield* fileSystem
          .exists(legacyPath)
          .pipe(Effect.mapError(toTerminalHistoryError("migrate", projectId, terminalId))))
      ) {
        return "";
      }
    ...
    const deleteHistory = Effect.fn("terminal.deleteHistory")(function* (
      ...
      yield* fileSystem.remove(historyPath(projectId, terminalId), { force: true }).pipe(
        Effect.catch((error) =>
      ...
      if (terminalId === DEFAULT_TERMINAL_ID) {
        yield* fileSystem.remove(legacyHistoryPath(projectId), { force: true }).pipe(
          Effect.catch((error) =>
            Effect.logWarning("failed to delete terminal history", {
              projectId,
              terminalId,
    ...
    const deleteAllHistoryForProject = Effect.fn("terminal.deleteAllHistoryForProject")(function* (
      projectId: string,
    ) {
      const projectPrefix = `${toSafeProjectId(projectId)}_`;
      ...
      yield* Effect.forEach(
        entries.filter(
          (name) =>
            name === `${toSafeProjectId(projectId)}.log` ||
            name === `${legacySafeProjectId(projectId)}.log` ||
            name.startsWith(projectPrefix),
        ),
        (name) =>
          fileSystem.remove(path.join(logsDir, name), { force: true }).pipe(
            Effect.catch((error) =>
              Effect.logWarning("failed to delete terminal histories for project", {
                projectId,
                error: error instanceof Error ? error.message : String(error),
              }),
            ),
    ...
    const getSession = Effect.fn("terminal.getSession")(function* (
      ...
      return yield* Effect.map(readManagerState, (state) =>
        Option.fromNullishOr(state.sessions.get(toSessionKey(projectId, terminalId))),
      );
    });
    ...
    const requireSession = Effect.fn("terminal.requireSession")(function* (
      ...
      return yield* Effect.flatMap(getSession(projectId, terminalId), (session) =>
    ...
    const sessionsForProject = Effect.fn("terminal.sessionsForProject")(function* (
      projectId: string,
    ) {
      return yield* readManagerState.pipe(
        Effect.map((state) =>
          [...state.sessions.values()].filter((session) => session.projectId === projectId),
        ),
      );
    });
      ...
      function* () {
        yield* modifyManagerState((state) => {
          ...
          for (const session of inactiveSessions.slice(0, toEvict)) {
            const key = toSessionKey(session.projectId, session.terminalId);
            sessions.delete(key);
          }
      ...
      while (true) {
        ...
        if (action.type === "output") {
          if (action.history !== null) {
            yield* queuePersist(action.projectId, action.terminalId, action.history);
          }
    ...
    const stopProcess = Effect.fn("terminal.stopProcess")(function* (
      ...
      yield* startKillEscalation(process, session.projectId, session.terminalId);
      yield* evictInactiveSessionsIfNeeded();
    });
    ...
    const trySpawn = Effect.fn("terminal.trySpawn")(function* (
      ...
      const attempt = yield* Effect.result(
        options.ptyAdapter.spawn({
          ...
          rows: session.rows,
          env: spawnEnv,
          label: `pty:${session.projectId}:${session.terminalId}`,
        }),
      );
    ...
    const startSession = Effect.fn("terminal.startSession")(function* (
      ...
      yield* Effect.annotateCurrentSpan({
        "terminal.project_id": session.projectId,
...

      {
        ...
        if (ptyProcess) {
          yield* startKillEscalation(ptyProcess, session.projectId, session.terminalId);
        }
    ...
    const closeSession = Effect.fn("terminal.closeSession")(function* (
      ...
      const key = toSessionKey(projectId, terminalId);
      const session = yield* getSession(projectId, terminalId);

      if (Option.isSome(session)) {
        yield* stopProcess(session.value);
        yield* persistHistory(projectId, terminalId, session.value.history);
      }
      ...
      if (deleteHistoryOnClose) {
        yield* deleteHistory(projectId, terminalId);
      }
    });
      ...
      const checkSubprocessActivity = Effect.fn("terminal.checkSubprocessActivity")(function* (
        ...
        const event = yield* modifyManagerState((state) => {
          const liveSession: Option.Option<TerminalSessionState> = Option.fromNullishOr(
            state.sessions.get(toSessionKey(session.projectId, session.terminalId)),
          );
          if (
          ...
          return [
            Option.some({
              ...
              projectId: liveSession.value.projectId,
    ...
    yield* Effect.addFinalizer(() =>
      Effect.gen(function* () {
        ...
        const cleanupSession = Effect.fn("terminal.cleanupSession")(function* (
          ...
          yield* runKillEscalation(session.process, session.projectId, session.terminalId);
        });
    ...
    const open: TerminalManagerShape["open"] = (input) =>
      withProjectLock(
        ...
        Effect.gen(function* () {
          ...
          const sessionKey = toSessionKey(input.projectId, terminalId);
          const existing = yield* getSession(input.projectId, terminalId);
          if (Option.isNone(existing)) {
            yield* flushPersist(input.projectId, terminalId);
            const history = yield* readHistory(input.projectId, terminalId);
    ...
    const write: TerminalManagerShape["write"] = Effect.fn("terminal.write")(function* (input) {
      const terminalId = input.terminalId ?? DEFAULT_TERMINAL_ID;
      const session = yield* requireSession(input.projectId, terminalId);
      const process = session.process;
    ...
    const clear: TerminalManagerShape["clear"] = (input) =>
      withProjectLock(
        ...
        Effect.gen(function* () {
          ...
          session.processEventDrainRunning = false;
          session.updatedAt = new Date().toISOString();
          yield* persistHistory(input.projectId, terminalId, session.history);
          yield* publishEvent({
            type: "cleared",
            projectId: input.projectId,
            terminalId,
    ...
    const restart: TerminalManagerShape["restart"] = (input) =>
      withProjectLock(
        input.projectId,
        Effect.gen(function* () {
          yield* increment(terminalRestartsTotal, { scope: "project" });
          const terminalId = input.terminalId ?? DEFAULT_TERMINAL_ID;
          yield* assertValidCwd(input.cwd);

          const sessionKey = toSessionKey(input.projectId, terminalId);
          const existingSession = yield* getSession(input.projectId, terminalId);
    ...
    const close: TerminalManagerShape["close"] = (input) =>
      withProjectLock(
        ...
        Effect.gen(function* () {
          if (input.terminalId) {
            yield* closeSession(input.projectId, input.terminalId, input.deleteHistory === true);
            return;
          }

          const projectSessions = yield* sessionsForProject(input.projectId);
          yield* Effect.forEach(
            projectSessions,
            (session) => closeSession(input.projectId, session.terminalId, false),
            { discard: true },
          );

          if (input.deleteHistory) {
            yield* deleteAllHistoryForProject(input.projectId);
          }
        }),
      );
