...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@codewithme/contracts",
    "--filter=@codewithme/web",
    "--filter=codewithme",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=codewithme"],
  "dev:web": ["run", "dev", "--filter=@codewithme/web"],
  "dev:desktop": [
    "run",
    "dev",
    "--filter=@codewithme/desktop",
    "--filter=@codewithme/web",
    "--parallel",
  ],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("CODEWITHME_PORT_OFFSET"),
  devInstance: optionalStringConfig("CODEWITHME_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid CODEWITHME_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `CODEWITHME_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric CODEWITHME_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed CODEWITHME_DEV_INSTANCE=${seed}` };
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly codewithmeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(codewithmeHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      CODEWITHME_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.CODEWITHME_PORT = String(serverPort);
      output.VITE_WS_URL = `ws://localhost:${serverPort}`;
    } else {
      delete output.CODEWITHME_PORT;
      delete output.VITE_WS_URL;
      delete output.CODEWITHME_AUTH_TOKEN;
      delete output.CODEWITHME_MODE;
      delete output.CODEWITHME_NO_BROWSER;
      delete output.CODEWITHME_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.CODEWITHME_HOST = host;
    }

    if (!isDesktopMode && authToken !== undefined) {
      output.CODEWITHME_AUTH_TOKEN = authToken;
    } else if (!isDesktopMode) {
      delete output.CODEWITHME_AUTH_TOKEN;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.CODEWITHME_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.CODEWITHME_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.CODEWITHME_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.CODEWITHME_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.CODEWITHME_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.CODEWITHME_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.CODEWITHME_MODE = "web";
      delete output.CODEWITHME_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read CODEWITHME_PORT_OFFSET/CODEWITHME_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("CODEWITHME_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv(
        "CODEWITHME_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
      ),
      logWebSocketEvents: readOptionalBooleanEnv("CODEWITHME_LOG_WS_EVENTS"),
    };
    ...
    const env = yield* createDevRunnerEnv({
      ...
      serverOffset,
      webOffset,
      codewithmeHome: input.codewithmeHome,
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.CODEWITHME_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.CODEWITHME_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  codewithmeHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all CodeWithMe data (equivalent to CODEWITHME_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("CODEWITHME_HOME")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to CODEWITHME_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("CODEWITHME_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to CODEWITHME_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("CODEWITHME_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to CODEWITHME_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("CODEWITHME_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription(
      "WebSocket event logging toggle (equivalent to CODEWITHME_LOG_WS_EVENTS).",
    ),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("CODEWITHME_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to CODEWITHME_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("CODEWITHME_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to CODEWITHME_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("CODEWITHME_PORT")),
  ),
