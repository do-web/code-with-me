...
import {
  ...
  type PersistedComposerDocumentAttachment,
...
import {
  ...
  readFileAsDataUrl,
  reconcileMountedTerminalProjectIds,
...
const DOCUMENT_EXTENSION_WHITELIST = new Set([
  ".pdf",
  ".doc",
  ".docx",
  ".rtf",
...
function classifyAttachmentFile(file: File): "image" | "document" | "text" {
  if (file.type.startsWith("image/")) return "image";
  if (file.type.length > 0 && CHAT_DOCUMENT_MIME_PATTERN.test(file.type)) return "document";
  if (DOCUMENT_EXTENSION_WHITELIST.has(fileExtensionLower(file.name))) return "document";
  return "text";
}

function inferDocumentMimeFromFile(file: File): string | null {
  if (file.type.length > 0 && CHAT_DOCUMENT_MIME_PATTERN.test(file.type)) return file.type;
  const extension = fileExtensionLower(file.name);
  return Object.hasOwn(DOCUMENT_MIME_BY_EXTENSION, extension)
    ? (DOCUMENT_MIME_BY_EXTENSION[extension] ?? null)
    : null;
}
  ...
  const serverAcknowledgedLocalDispatch = useMemo(
    () =>
      hasServerAcknowledgedLocalDispatch({
        ...
        queueItemsCount: (input.activeThread?.queueItems ?? []).length,
      }),
    [
      input.activeLatestTurn,
      input.activePendingApproval,
      input.activePendingUserInput,
      input.activeThread?.queueItems,
...
function PersistentThreadTerminalDrawer({
  ...
  const terminalState = useTerminalStateStore((state) =>
    selectProjectTerminalState(state.terminalStateByProjectId, projectId),
  );
  const storeSetTerminalHeight = useTerminalStateStore((state) => state.setTerminalHeight);
  const storeToggleTerminalCollapsed = useTerminalStateStore(
    (state) => state.toggleTerminalCollapsed,
  );
  const collapsedInfo = useTerminalStateStore((state) =>
    selectTerminalCollapsedInfo(
  ...
  const setTerminalHeight = useCallback(
    (height: number) => {
      if (!projectId) return;
      storeSetTerminalHeight(projectId, height);
    },
    [storeSetTerminalHeight, projectId],
  );

  const toggleCollapsed = useCallback(() => {
    if (!projectId) return;
    storeToggleTerminalCollapsed(projectId);
  }, [storeToggleTerminalCollapsed, projectId]);

  const splitTerminal = useCallback(() => {
    if (!projectId) return;
    storeSplitTerminal(projectId, `terminal-${randomUUID()}`);
    bumpFocusRequestId();
  }, [bumpFocusRequestId, storeSplitTerminal, projectId]);

  const createNewTerminal = useCallback(() => {
    if (!projectId) return;
    storeNewTerminal(projectId, `terminal-${randomUUID()}`);
    bumpFocusRequestId();
  }, [bumpFocusRequestId, storeNewTerminal, projectId]);

  const activateTerminal = useCallback(
    (terminalId: string) => {
      if (!projectId) return;
      storeSetActiveTerminal(projectId, terminalId);
      bumpFocusRequestId();
    },
    [bumpFocusRequestId, storeSetActiveTerminal, projectId],
  );
  ...
  const closeTerminal = useCallback(
    (terminalId: string) => {
      ...
      const fallbackExitWrite = () =>
        api.terminal.write({ projectId, terminalId, data: "exit\n" }).catch(() => undefined);

      if ("close" in api.terminal && typeof api.terminal.close === "function") {
        void (async () => {
          if (isFinalTerminal) {
            await api.terminal.clear({ projectId, terminalId }).catch(() => undefined);
          }
      ...
      storeCloseTerminal(projectId, terminalId);
      bumpFocusRequestId();
    },
    [bumpFocusRequestId, storeCloseTerminal, terminalState.terminalIds.length, projectId],
  );
  ...
  return (
    <div className={visible ? undefined : "hidden"}>
      <ThreadTerminalDrawer
        ...
        visible={visible}
        collapsed={terminalState.terminalCollapsed}
...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const composerSendState = useMemo(
    () =>
      deriveComposerSendState({
        prompt,
        imageCount: composerImages.length + composerDocuments.length,
  ...
  const addComposerDraftDocument = useComposerDraftStore((store) => store.addDocument);
  ...
  const syncComposerDraftPersistedDocumentAttachments = useComposerDraftStore(
    (store) => store.syncPersistedDocumentAttachments,
  );
  const clearComposerDraftPersistedDocumentAttachments = useComposerDraftStore(
    (store) => store.clearPersistedDocumentAttachments,
  );
  ...
  const terminalOpenByProjectRef = useRef<Record<string, boolean>>({});
  ...
  const terminalState = useMemo(
    () =>
      selectProjectTerminalState(terminalStateByProjectId, activeProjectId ?? ("" as ProjectId)),
    [terminalStateByProjectId, activeProjectId],
  );
  const openTerminalProjectIds = useMemo(
    () =>
      Object.entries(terminalStateByProjectId).flatMap(([nextProjectId, nextTerminalState]) =>
        nextTerminalState.terminalOpen ? [nextProjectId as ProjectId] : [],
      ),
  ...
  const storeServerTerminalLaunchContext = useTerminalStateStore((s) =>
    activeProjectId ? (s.terminalLaunchContextByProjectId[activeProjectId] ?? null) : null,
  );
  const storeClearTerminalLaunchContext = useTerminalStateStore(
    (s) => s.clearTerminalLaunchContext,
  );
  const threads = useStore((state) => state.threads);
  const [mountedTerminalProjectIds, setMountedTerminalProjectIds] = useState<ProjectId[]>([]);
  ...
  // openTerminalProjectIds is used directly for mounting — no thread→project reverse mapping needed.
  ...
  useEffect(() => {
    setMountedTerminalProjectIds((currentProjectIds) => {
      const nextProjectIds = reconcileMountedTerminalProjectIds({
        currentProjectIds,
        openProjectIds: openTerminalProjectIds,
        activeProjectId,
        activeProjectTerminalOpen: Boolean(activeProjectId && terminalState.terminalOpen),
      });
      return currentProjectIds.length === nextProjectIds.length &&
        currentProjectIds.every((nextProjectId, index) => nextProjectId === nextProjectIds[index])
        ? currentProjectIds
        : nextProjectIds;
    });
  }, [activeProjectId, openTerminalProjectIds, terminalState.terminalOpen]);
  ...
  // Lock the provider picker only while a turn is actively being processed.
  // Idle/ready/closed/error sessions can be switched to a different provider.
  ...
  const lockedProvider: ProviderKind | null =
    hasThreadStarted && turnInProgress
  ...
  const timelineEntries = useMemo(
    ...
    [activeThread?.proposedPlans, activeThread?.queueItems, timelineMessages, workLogEntries],
  );
  ...
  const packageScriptsQuery = useQuery(
    packageScriptsQueryOptions({ cwd: gitCwd ?? activeProject?.cwd ?? null }),
  );
  ...
  const activeTerminalLaunchContext =
    terminalLaunchContext?.projectId === activeProjectId
  ...
  const setTerminalOpen = useCallback(
    (open: boolean) => {
      ...
      storeSetTerminalOpen(activeProject.id, open);
    },
    [activeProject, storeSetTerminalOpen],
  );
  const toggleTerminalVisibility = useCallback(() => {
    if (!activeProject) return;
    setTerminalOpen(!terminalState.terminalOpen);
  }, [activeProject, setTerminalOpen, terminalState.terminalOpen]);
  const splitTerminal = useCallback(() => {
    if (!activeProject || hasReachedSplitLimit) return;
    const terminalId = `terminal-${randomUUID()}`;
    storeSplitTerminal(activeProject.id, terminalId);
    setTerminalFocusRequestId((value) => value + 1);
  }, [activeProject, hasReachedSplitLimit, storeSplitTerminal]);
  const createNewTerminal = useCallback(() => {
    if (!activeProject) return;
    const terminalId = `terminal-${randomUUID()}`;
    storeNewTerminal(activeProject.id, terminalId);
    setTerminalFocusRequestId((value) => value + 1);
  }, [activeProject, storeNewTerminal]);
  const closeTerminal = useCallback(
    (terminalId: string) => {
      const api = readNativeApi();
      ...
      const fallbackExitWrite = () =>
        api.terminal
          .write({ projectId: activeProject.id, terminalId, data: "exit\n" })
          .catch(() => undefined);
      if ("close" in api.terminal && typeof api.terminal.close === "function") {
        void (async () => {
          if (isFinalTerminal) {
            await api.terminal
              .clear({ projectId: activeProject.id, terminalId })
              .catch(() => undefined);
          }
          await api.terminal.close({
      ...
      storeCloseTerminal(activeProject.id, terminalId);
      setTerminalFocusRequestId((value) => value + 1);
    },
    [activeProject, storeCloseTerminal, terminalState.terminalIds.length],
  );
  const runProjectScript = useCallback(
    async (
      ...
      if (shouldCreateNewTerminal) {
        storeNewTerminal(activeProject.id, targetTerminalId);
      } else {
        storeSetActiveTerminal(activeProject.id, targetTerminalId);
      }
      setTerminalFocusRequestId((value) => value + 1);
  ...
  const executeInTerminal = useCallback(
    async (command: string) => {
      ...
      const baseTerminalId =
        ...
        DEFAULT_THREAD_TERMINAL_ID;
      const isBaseTerminalBusy = terminalState.runningTerminalIds.includes(baseTerminalId);
      const targetTerminalId = isBaseTerminalBusy ? `terminal-${randomUUID()}` : baseTerminalId;
      ...
      const openTerminalInput: TerminalOpenInput = {
        ...
        ...(targetWorktreePath !== null ? { worktreePath: targetWorktreePath } : {}),
        env: runtimeEnv,
        ...(isBaseTerminalBusy ? { cols: SCRIPT_TERMINAL_COLS, rows: SCRIPT_TERMINAL_ROWS } : {}),
      };
  ...
  }, [composerDocuments, composerImages]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      if (composerDocuments.length === 0) {
        clearComposerDraftPersistedDocumentAttachments(threadId);
        return;
      }
      const getPersistedDocumentsForThread = () =>
        useComposerDraftStore.getState().draftsByThreadId[threadId]?.persistedDocumentAttachments ??
        [];
      try {
        const currentPersisted = getPersistedDocumentsForThread();
        const existingPersistedById = new Map(
          currentPersisted.map((attachment) => [attachment.id, attachment]),
        );
        const stagedById = new Map<string, PersistedComposerDocumentAttachment>();
        await Promise.all(
          composerDocuments.map(async (doc) => {
            try {
              const dataUrl = await readFileAsDataUrl(doc.file);
              stagedById.set(doc.id, {
                id: doc.id,
                name: doc.name,
              ...
              const existing = existingPersistedById.get(doc.id);
              if (existing) {
                stagedById.set(doc.id, existing);
              }
            }
          }),
        );
        const serialized = Array.from(stagedById.values());
        if (cancelled) {
          return;
        }
        syncComposerDraftPersistedDocumentAttachments(threadId, serialized);
      } catch {
        if (cancelled) return;
        syncComposerDraftPersistedDocumentAttachments(threadId, getPersistedDocumentsForThread());
      }
    })();
    return () => {
      cancelled = true;
    ...
    clearComposerDraftPersistedDocumentAttachments,
    composerDocuments,
    syncComposerDraftPersistedDocumentAttachments,
    threadId,
  ]);
  ...
  useEffect(() => {
    ...
    setTerminalLaunchContext((current) => {
      ...
      if (current.projectId === activeProjectId) return current;
      return null;
    });
  }, [activeProjectId]);

  useEffect(() => {
    if (!activeProjectId || !activeProjectCwd) {
      return;
    }
    setTerminalLaunchContext((current) => {
      ...
      if (
        ...
        storeClearTerminalLaunchContext(activeProjectId);
        return null;
      }
      return current;
    });
  ...
  useEffect(() => {
    if (!activeProjectId || !activeProjectCwd || !storeServerTerminalLaunchContext) {
      return;
    }
    const settledCwd = projectScriptCwd({
      project: { cwd: activeProjectCwd },
  ...
  useEffect(() => {
    ...
    setTerminalLaunchContext((current) =>
      current?.projectId === activeProjectId ? null : current,
    );
  }, [activeProjectId, storeClearTerminalLaunchContext, terminalState.terminalOpen]);
  ...
  useEffect(() => {
    ...
    const previous = terminalOpenByProjectRef.current[activeProjectId] ?? false;
    const current = Boolean(terminalState.terminalOpen);

    if (!previous && current) {
      terminalOpenByProjectRef.current[activeProjectId] = current;
      setTerminalFocusRequestId((value) => value + 1);
      return;
    ...
    terminalOpenByProjectRef.current[activeProjectId] = current;
  }, [activeProjectId, focusComposer, terminalState.terminalOpen]);
  ...
  const addComposerAttachments = (files: File[]) => {
    ...
    for (const file of files) {
      ...
      if (kind === "image") {
        if (file.size > PROVIDER_SEND_TURN_MAX_IMAGE_BYTES) {
          error = `'${file.name}' exceeds the ${IMAGE_SIZE_LIMIT_LABEL} image attachment limit.`;
          continue;
        }
      ...
      if (kind === "document") {
        if (runtimeMode !== "full-access") {
          error = `Document attachments require full-access runtime (currently ${runtimeMode}).`;
          continue;
        }
        const mimeType = inferDocumentMimeFromFile(file);
        if (!mimeType) {
          nextFiles.push({ id: randomUUID(), name: file.name, file });
          continue;
        }
        if (file.size > PROVIDER_SEND_TURN_MAX_DOCUMENT_BYTES) {
          error = `'${file.name}' exceeds the ${DOCUMENT_SIZE_LIMIT_LABEL} document attachment limit.`;
          continue;
        }
        if (nextAttachmentCount >= PROVIDER_SEND_TURN_MAX_ATTACHMENTS) {
          error = `You can attach up to ${PROVIDER_SEND_TURN_MAX_ATTACHMENTS} files per message.`;
    ...
    if (nextDocuments.length === 1 && nextDocuments[0]) {
      addComposerDraftDocument(activeThreadId, nextDocuments[0]);
    } else if (nextDocuments.length > 1) {
      addComposerDraftDocuments(activeThreadId, nextDocuments);
    }
    ...
    await (async () => {
      ...
      if (
        ...
        if (composerDocumentsSnapshot.length > 0) {
          addComposerDraftDocuments(threadIdForSend, composerDocumentsSnapshot);
        }
  ...
  const onEditQueueItem = useCallback(
    (queueItemId: MessageId, nextText: string) => {
      const api = readNativeApi();
      if (!api || !activeThread) return;
      void api.orchestration
        .dispatchCommand({
          type: "thread.queue-item.edit",
          commandId: newCommandId(),
          threadId: activeThread.id,
          queueItemId,
          text: nextText,
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden bg-background">
      ...
      >
        <ChatHeader
          ...
          packageScripts={packageScriptsQuery.data?.scripts ?? null}
          packageManager={packageScriptsQuery.data?.packageManager ?? null}
          ...
          onRunPackageScript={(cmd) => void executeInTerminal(cmd)}
                      ...
                      (composerImages.length > 0 ||
                        ...
                        <div className="mb-3 flex flex-wrap gap-2">
                          ...
                          {composerDocuments.map((doc) => (
                            <div
                              key={doc.id}
                              className="relative flex h-16 items-center gap-1.5 overflow-hidden rounded-lg border border-border/80 bg-background pl-2 pr-7"
                              title={`${doc.name} (${doc.mimeType})`}
                            >
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  removeComposerDraftDocument(activeThreadId, doc.id);
                                }}
      ...
      {mountedTerminalProjectIds.map((mountedProjectId) => (
        <PersistentThreadTerminalDrawer
          key={mountedProjectId}
          projectId={mountedProjectId}
          visible={mountedProjectId === activeProjectId && terminalState.terminalOpen}
          launchContext={
            mountedProjectId === activeProjectId ? (activeTerminalLaunchContext ?? null) : null
          }
          focusRequestId={mountedProjectId === activeProjectId ? terminalFocusRequestId : 0}
