...
import { parsePersistedServerObservabilitySettings } from "@codewithme/shared/serverSettings";
...
const BootstrapEnvelopeSchema = Schema.Struct({
  ...
  codewithmeHome: Schema.optional(Schema.String),
...
const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription("Base directory path (equivalent to CODEWITHME_HOME)."),
  Flag.optional,
);
...
const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to CODEWITHME_LOG_WS_EVENTS).",
  ),
...
const EnvServerConfig = Config.all({
  logLevel: Config.logLevel("CODEWITHME_LOG_LEVEL").pipe(Config.withDefault("Info")),
  traceMinLevel: Config.logLevel("CODEWITHME_TRACE_MIN_LEVEL").pipe(Config.withDefault("Info")),
  traceTimingEnabled: Config.boolean("CODEWITHME_TRACE_TIMING_ENABLED").pipe(
    Config.withDefault(true),
  ),
  traceFile: Config.string("CODEWITHME_TRACE_FILE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  traceMaxBytes: Config.int("CODEWITHME_TRACE_MAX_BYTES").pipe(
    Config.withDefault(10 * 1024 * 1024),
  ),
  traceMaxFiles: Config.int("CODEWITHME_TRACE_MAX_FILES").pipe(Config.withDefault(10)),
  traceBatchWindowMs: Config.int("CODEWITHME_TRACE_BATCH_WINDOW_MS").pipe(Config.withDefault(200)),
  otlpTracesUrl: Config.string("CODEWITHME_OTLP_TRACES_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpMetricsUrl: Config.string("CODEWITHME_OTLP_METRICS_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpExportIntervalMs: Config.int("CODEWITHME_OTLP_EXPORT_INTERVAL_MS").pipe(
    Config.withDefault(10_000),
  ),
  otlpServiceName: Config.string("CODEWITHME_OTLP_SERVICE_NAME").pipe(
    Config.withDefault("codewithme-server"),
  ),
  mode: Config.schema(RuntimeMode, "CODEWITHME_MODE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  port: Config.port("CODEWITHME_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("CODEWITHME_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  codewithmeHome: Config.string("CODEWITHME_HOME").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: Config.boolean("CODEWITHME_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  authToken: Config.string("CODEWITHME_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  bootstrapFd: Config.int("CODEWITHME_BOOTSTRAP_FD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("CODEWITHME_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("CODEWITHME_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* resolveBaseDir(
      Option.getOrUndefined(
        resolveOptionPrecedence(
          flags.baseDir,
          Option.fromUndefinedOr(env.codewithmeHome),
          Option.flatMap(bootstrapEnvelope, (bootstrap) =>
            Option.fromUndefinedOr(bootstrap.codewithmeHome),
          ),
        ),
      ),
    );
...
const rootCommand = Command.make("codewithme", commandFlags).pipe(
  Command.withDescription("Run the CodeWithMe server."),
